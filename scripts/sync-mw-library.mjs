...
import { MW_LIBRARY_BLUEPRINT, WARMUP_BLUEPRINT, COOLDOWN_BLUEPRINT, BADMINTON_WARMUP_BLUEPRINT, BADMINTON_COOLDOWN_BLUEPRINT } from './mw-library-blueprint.mjs';
...
async function main() {
  ...
  for (const item of BADMINTON_WARMUP_BLUEPRINT) {
    const step = await resolveStep(client, item);
    badmintonWarmup.push(step);
    resolved.push({ module: 'badminton_warmup', query: item.query, matched: step.source.name });
  }

  const badmintonCooldown = [];
  for (const item of BADMINTON_COOLDOWN_BLUEPRINT) {
    const step = await resolveStep(client, item);
    badmintonCooldown.push(step);
    resolved.push({ module: 'badminton_cooldown', query: item.query, matched: step.source.name });
  }
