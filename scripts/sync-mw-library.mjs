...
import { MW_LIBRARY_BLUEPRINT } from './mw-library-blueprint.mjs';
...
async function resolveExercise(client, item) {
  const endpoint = `/search?q=${encodeURIComponent(item.query)}&limit=8`;
  const data = await client.getJson(endpoint);
  const results = Array.isArray(data) ? data : data.results || [];
  const match = pickBestMatch(results, item.query);
  if (!match) {
    throw new Error(`No exercise matched query: ${item.query}`);
  }
